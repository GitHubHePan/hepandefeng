<template>
  <div class="content-page">
      ...
      <div class="form-table-box">
        <el-tabs v-model="activeName" @tab-click="handleClick">
          <el-tab-pane label="订单信息" name="first">
            ...
            <el-table :data="addressData" style="width: 100%" height="700" border stripe>
              ...
              <el-table-column prop="mobile" label="客户手机" width="120" />
              <el-table-column prop="address" label="客户地址" />
              <el-table-column prop="postscript" label="买家备注" width="300" />
            ...
            <el-table :data="infoForm.goodsList" style="width: 100%" border stripe>
              <el-table-column prop="goods_sn" label="商品SKU" width="120" />
              ...
              <el-table-column prop="goods_specifition_name_value" label="型号" width="100" />
    ...
    <el-dialog title="修改地址" v-model:visible="dialogAddressVisible">
      <el-form :model="nowAddressData">
        ...
        <el-form-item label="详细地址:" label-width="120px">
          <el-input class="el-input" v-model="nowAddressData.cAddress" auto-complete="off" placeholder="请输入详细地" />
        </el-form-item>
        <el-form-item label="收货人:" label-width="120px">
          <el-input class="el-input" v-model="nowAddressData.name" auto-complete="off" placeholder="请输入收货人" />
        </el-form-item>
    ...
    <el-dialog title="修改商品" v-model:visible="dialogGoodsListVisible">
      <el-form :model="goodsData">
        ...
        <el-form-item label="购买数量:" label-width="120px">
          <el-input-number v-model="goodsData.number" :min="1" label="购买的数量" />
        </el-form-item>
      </el-form>
      <template v-slot:footer>
        <div class="dialog-footer">
    ...
    <el-dialog title="变更状态" v-model:visible="statusVisible" width="30%">
...
export default {
  ...
  methods: {
    ...
    handleClick(tab, event) {
      ...
      if (pindex == 1) {
        if (this.is_finish == 0) {
          this.on_posting = 1
          getOrderExpressApi({ orderId: this.infoForm.id }).then((response) => {
            this.expressData = response.data
            this.expressData.traces = JSON.parse(this.expressData.traces)
            this.is_finish = response.data.is_finish
            this.on_posting = 0
          })
        }
      }
    ...
    handleRowDelete(index, row) {
      ...
      this.$confirm("确定要删除?", "提示", {
        ...
        .then(() => {
          goodsListDeleteApi(this.goodsData).then((response) => {
    ...
    saveGoodsList() {
      ...
      if (old != now) {
        ...
        saveGoodsListApi(this.goodsData).then((response) => {
          //                        console.log(response.data);
          //                        this.dialogGoodsListVisible = false;
          //                        this.infoForm.order_sn = response.data;
          //
          //                        this.addressData = [];
          //                        this.getInfo();
          this.$router.go(-1)
    ...
    saveAddress() {
      ...
      saveAddressDataApi(this.nowAddressData).then((response) => {
        console.log("++---------------------------++")
        console.log(response)
        console.log("++---------------------------++")
        if (response.data.errno === 0) {
  ...
  mounted() {
    ...
    this.infoForm.id = this.$route.params.id || 0
    this.getInfo()
    this.getAllRegion()
  }
}
